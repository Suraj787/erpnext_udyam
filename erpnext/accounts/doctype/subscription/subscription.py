...
class Subscription(Document):
	...
	def get_billing_cycle_data(self):
		""".utils import flt, nowdate, add_days, cint
	...
	def validate(self):
		...
		# plan=self.plans[0].plan
		# if not self.next_scheduled_date:
		# 	frq=frappe.db.get_value('Subscription Plan',plan,'frequency')
		# 	rp=frappe.db.get_value('Subscription Plan',plan,'repeat_on_day')
		# 	next_date = get_next_schedule_date(self.start,frq,rp )
		# 	print(next_date)
		# 	end_date=datetime.strptime(self.current_invoice_end, '%Y-%m-%d').date()
		# 	if next_date<end_date:
		# 		self.next_scheduled_date=next_date
		prorate = frappe.db.get_single_value('Subscription Settings', 'prorate')
		# print(prorate)
		# if self.next_scheduled_date = frappe.utils.nowdate()
	def validate_subscription_plans(self):
		for d in frappe.db.sql("""
			select
				A.name,A.start
			from `tabSubscription` A left join `tabSubscription Plan Detail` B on
			A.name=B.parent
			where A.Customer = %(customer)s and A.docstatus < 2
			and A.start >= %(start)s 
			and A.name != %(name)s and B.plan=%(plan)s""", {
	...
	def throw_overlap_error(self, d):
		msg = _("Customer" + """ <b><a href="#Form/Customer/{0}">{0}</a></b>"""+"has already linked with" + """ <b><a href="#Form/Subscription/{1}">{1}</a></b>""" ).format(self.customer,
			d['name'])
			#  \
			# + """ <b><a href="#Form/Attendance Request/{0}">{0}</a></b>""".format(d["name"])
		frappe.msgprint(msg)
	...
	# def generate_delivery(self, prorate=0):
	# 	"""
	# 	Creates a `Sales Invoice` for the `Subscription`, updates `self.invoices` and
	# 	saves the `Subscription`.
	# 	"""
	# 	print('###################')
	# 	self.create_delivery(prorate)
	# 	# self.append('deliveries', {'delivery': delivery.name})
	# 	# self.save()

	# 	# return delivery

	# def create_delivery(self, prorate):
	# 	"""
	# 	Creates a `Sales Invoice`, submits it and returns it
	# 	"""
	# 	print('****************1')
	...
	# 	delivery.posting_date = self.start
	# 	delivery.set_warehouse=self.warehouse_location
	...
	# 	print('****************3')
	# 	if self.tax_template:
	# 		delivery.taxes_and_charges = self.tax_template
	# 		delivery.set_taxes()
	...
	# 	# Discounts
	# 	if self.additional_discount_percentage:
	# 		delivery.additional_discount_percentage = self.additional_discount_percentage

	# 	if self.additional_discount_amount:
	# 		delivery.discount_amount = self.additional_discount_amount
	...
	# 	# Subscription period
	# 	delivery.from_date = self.current_invoice_start
	# 	delivery.to_date = self.current_invoice_end
...
@frappe.whitelist()
def cancel_multiple_subscription(name):
	name=name.replace('[','')
	name=name.replace(']','')
	name=name.split(',')
	for n in name:
		subscription = frappe.get_doc('Subscription', n.replace('"',''))
		if subscription.status != 'Cancelled':
			subscription.status = 'Cancelled'
			subscription.cancelation_date = nowdate()
			subscription.save()
